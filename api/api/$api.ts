        ...
        reserve: {
          _id: (val3: number) => {
            ...
            return {
               ...
               */
              patch: (option?: { config?: T | undefined } | undefined) =>
                fetch<void, BasicHeaders, Methods21['patch']['status']>(prefix, prefix3, PATCH, option).send(),
              /**
               * 予約を更新する
               */
              $patch: (option?: { config?: T | undefined } | undefined) =>
                fetch<void, BasicHeaders, Methods21['patch']['status']>(prefix, prefix3, PATCH, option).send().then(r => r.body),
              $path: () => `${prefix}${prefix3}`
            }
          }
        },
